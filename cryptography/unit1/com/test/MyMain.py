# Decode the two ciphertexts from the Instructors Box below, 
# or the C1, C2 variables - which are the same
#
# We highly recommend that you run your decoding code in the 
# programming language of your choice outside of the 
# this environment, as this system does not provide enough 
# computational resources to successfully decode
#
# After decoding the two ciphertexts, 
# replace the plaintext1 and plaintext2 variables below
# with the decoded ciphertexts
from mako.filters import is_ascii_str

# C1 and C2 are messages in english, 
# encoded using string_to_bits, with 7bit ASCII
# and then XOR'd with a secret key
#
# In pseudo-code:
# C1 = XOR(string_to_bits(plaintext1), secret_key)
# C2 = XOR(string_to_bits(plaintext2), secret_key)

BITS = ('0', '1')
ASCII_BITS = 7

LEN = 121
K = ""
C1 = ""
C2 = ""
assert len(K) == len(C1) == len(C2) == LEN * ASCII_BITS

plaintext1 = "decoded message"
plaintext2 = "the other decoded message"


def display_bits(b):
    """converts list of {0, 1}* to string"""
    return ''.join([BITS[e] for e in b])

def seq_to_bits(seq):
    return [0 if b == '0' else 1 for b in seq]

def pad_bits(bits, pad):
    """pads seq with leading 0s up to length pad"""
    assert len(bits) <= pad
    return [0] * (pad - len(bits)) + bits
        
def convert_to_bits(n):
    """converts an integer `n` to bit array"""
    result = []
    if n == 0:
        return [0]
    while n > 0:
        result = [(n % 2)] + result
        n = n / 2
    return result

def string_to_bits(s):
    def chr_to_bit(c):
        return pad_bits(convert_to_bits(ord(c)), ASCII_BITS)
    return [b for group in 
            map(chr_to_bit, s)
            for b in group]

def bits_to_char(b):
    assert len(b) == ASCII_BITS
    value = 0
    for e in b:
        value = (value * 2) + e
    return chr(value)

def list_to_string(p):
    return ''.join(p)

def bits_to_string(b):
    return ''.join([bits_to_char(b[i:i + ASCII_BITS]) 
                    for i in range(0, len(b), ASCII_BITS)])

def xor_bit_array_str(a, b):
    assert len(a) == len(b)
    return "".join(["%x" % (int(x,16) ^ int(y,16)) for (x, y) in zip(a, b)])
#    return display_bits(xor_bit_array(seq_to_bits(s1), seq_to_bits(s2)))

def try_string(cypher, word, anotherCypher):
    assert len(cypher) == LEN * ASCII_BITS
    maxPadding = LEN - len(word)
    for index in range(0, maxPadding):
        teststr = ''
        for bi in range (0, index):
            teststr += ' '
        teststr += word
        endIndex = index + len(word)
        for ai in range (endIndex, LEN):
            teststr += ' '
        teststr = display_bits(string_to_bits(teststr))
        assert len(teststr) == LEN * ASCII_BITS
        testKey = xor_bit_array_str(teststr, cypher)
        anotherMessage = xor_bit_array_str(testKey, anotherCypher)
        anotherMessageText = bits_to_string(seq_to_bits(anotherMessage))
        isText = True
        for c in anotherMessageText[index:endIndex]:
            if not c in ' "\'(),.;?abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ':
                isText = False
                break
        if isText:
            print 'A possible match found, part of other string: ' + anotherMessageText[index:endIndex]
            print 'Key[' + str(index * ASCII_BITS) + ':' + str(endIndex * ASCII_BITS) + ']: ' + testKey[index * ASCII_BITS:endIndex * ASCII_BITS]
    
if __name__ == '__main__':
    m1 = plaintext1
    g1 = xor_bit_array_str(C1, K)
    m2 = plaintext2
    g2 = xor_bit_array_str(C2, K)
    print 'G1 Bits [' + str(len(g1)) + ']: ' + display_bits(string_to_bits(g1))
    print 'G2 Bits [' + str(len(g2)) + ']: ' + display_bits(string_to_bits(g2))
    
    testStr = 'the'
    try_string(C1, testStr, C2)
    try_string(C1, testStr + ' ', C2)
    try_string(C1, ' ' + testStr, C2)
    try_string(C1, ' ' + testStr + ' ', C2)
