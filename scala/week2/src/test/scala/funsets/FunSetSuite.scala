...
class FunSetSuite extends FunSuite {
  ...
  test("contains is implemented") {
    ...
    assert(contains(x => x % 2 == 0, 12))
    assert(contains(x => x % 2 == 0, -20))
    assert(!contains(x => x % 2 == 0, -3))
    assert(!contains(x => x % 2 == 0, 15))
  }
  ...
  new TestSets {
    test("forall is implemented") {
      val s = (x: Int) => x % 10 == 0
      assert(forall(s, x => x % 5 == 0), "forall 1")
  ...
  new TestSets {
    test("exists is implemented") {
      ...
      assert(exists(s, x => x % 5 == 0), "exists 2")
    }
  }
  ...
  new TestSets {
    test("map is implemented") {
      val s = (x: Int) => true
      val t = map(s, x => x * 2)
      assert(forall(t, x => x % 2 == 0), "map 1")
      assert(!exists(t, x => x % 2 == 1), "map 2")
    }
  }
}
